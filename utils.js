...
const Utils = module.exports = { // Public functions accessible by other modules
  ...
  *********************************************************************/
  ...
  /********************************************************************
  Check if variable is Object Type {} (null is also an object)
  ...
  *********************************************************************/
  roundWithCascading: function(num, digits_after_decimal, safety = 10){
    ...
    for(var i = safety; i >= digits_after_decimal; i--){
  ...
  *********************************************************************/
  deepCopyObject: function(obj){
    ...
    // Always use polyfill instead of Structuredclone due to bug in browser
    return _Utils.deepCopyObjectPolyfill(obj);

    // --No NOT Proceed--

    // Use structuredClone() if available in this JS engine
    if( typeof structuredClone === 'function' ){
      return structuredClone(obj);
    }

    // Otherwise use Polyfill
    else{
      return _Utils.deepCopyObjectPolyfill(obj);
    }
  ...
  *********************************************************************/
  ...
  @return {Boolean} url_data - false if invalid URL
  @return {Map} url_data - URL Data Set if valid URL
  * @return {String} origin - Origin of the URL. Scheme+Domain+Port ('https://subdomain.example.com:8080')
  * @return {String} protocol - Protocol of the URL, including ':' ('https:')
  * @return {String} username - Username specified before the domain name ('user')
  * @return {String} password - Password specified before the domain name ('pass')
  * @return {String} hostname - Domain of the URL, including any subdomains ('subdomain.example.com')
  * @return {String} host - Hostname+Port ('subdomain.example.com:8080')
  * @return {Integer} port - Port Number ('8080')
  * @return {String} pathname - initial '/' followed by the path of the URL, not including the query string or fragment ('/abc/pqr/query')
  * @return {String} search - Querystring params, beginning with the leading ? character ('?param1=apple')
  * @return {String} hash - Fragment, beginning with the leading # character ('#section1')
  *********************************************************************/
  ...
  @param {String} pathname - initial '/' followed by the path of the URL, not including the query string or fragment ('/abc/pqr/query')

  @return {String, String[]} route_data - First value in path. Ex: 'abc'
  * @return {String} route - First value in path. Ex: 'abc'
  * @return {String[]} values - Array of subsequent values in path. Ex: ['pqr','query']
  *********************************************************************/
  ...
  *********************************************************************/
  convertCsvToData: function(csv_data){
    ...
    let header = Utils.splitWithTrim(
  ...
  *********************************************************************/
  convertDataToCsv: function(records){
    ...
    records.forEach(function(record){
      ...
      let row = fields.map(function(field){
...
const _Utils = { // Private functions accessible within this modules only
  ...
  It uses recursion to handle nested objects and arrays. It also checks the type of the input object using instanceof and typeof operators.
  Keep in mind that this function only works for plain JavaScript objects and arrays. It will not work for class instances or other non-plain objects.
